...
class AddHargaTitip :  UIViewController{
    override func viewDidLoad() {
        super.viewDidLoad()
        view.backgroundColor = UIColor.white
        
        if PostTitipJuga.varHarga.status != 0 {
            hargaText.text = PostTitipJuga.varHarga.harga
    ...
    @objc func handleSubmit(){
        ...
        if hargaText.text != ""{
            PostTitipJuga.varHarga.harga = hargaText.text!
            PostTitipJuga.varHarga.status = 1
            
            print(PostTitipJuga.varHarga.harga.self)
