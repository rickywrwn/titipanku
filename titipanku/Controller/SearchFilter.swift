...
class SearchFilter: UIViewController,UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(SearchFilter.keyboardWillShow), name: NSNotification.Name.UIKeyboardWillShow, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(SearchFilter.keyboardWillHide), name: NSNotification.Name.UIKeyboardWillHide, object: nil)
    ...
    @objc func handleApply(){
        ...
        if itung == 1{
            SearchRequest.varFilter.search = "negara"
            SearchRequest.varFilter.negara = beratText.text!
        }else if itung == 2 {
            SearchRequest.varFilter.search = "kategori"
            SearchRequest.varFilter.kategori = ukuranText.text!
        }else if itung == 3 {
            SearchRequest.varFilter.search = "none"
        }else{
            SearchRequest.varFilter.search = "semua"
            SearchRequest.varFilter.kategori = ukuranText.text!
            SearchRequest.varFilter.negara = beratText.text!
        }
        
        NotificationCenter.default.post(name: NSNotification.Name(rawValue: "reloadRequest"), object: nil)
