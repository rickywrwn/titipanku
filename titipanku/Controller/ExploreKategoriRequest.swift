...
class ExploreKategoriRequest: UICollectionViewController, UICollectionViewDelegateFlowLayout{
    ...
    func fetchRequests(_ completionHandler: @escaping ([App]) -> ()) {
        if let negara = isiData?.nama{
            let urlString = "http://titipanku.xyz/api/GetExploreKategoriRequest.php?negara=\(String(describing: negara))"
