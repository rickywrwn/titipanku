...
class UserPenjualanRequest: UIViewController, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout{
        ...
        if let emailNow : String = UserController.emailUser.email {
            ...
            URLSession.shared.dataTask(with: URL(string: urlString)!, completionHandler: { (data, response, error) -> Void in
                ...
                do {
                    ...
                    self.requests = try decoder.decode([VarOffer].self, from: data)
                    print(self.requests)
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        //SKActivityIndicator.show("Loading...", userInteractionStatus: false)
        let urlString = "http://titipanku.xyz/api/DetailBarang.php?id=\(requests[indexPath.row].idRequest)"
